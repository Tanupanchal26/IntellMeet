...
export const useChat = (meetingId: string) => {
  ...
  useEffect(() => {
    ...
    const onTyping = ({ name, isTyping }: { name: string; isTyping: boolean }) => setTyping(name, isTyping);
  ...
  }, [meetingId, socket, addMessage, setTyping]);
  ...
  const sendTyping = (name: string, isTyping: boolean) => {
    socket?.emit('chat:typing', { meetingId, name, isTyping });
  };
